...
def _get_binning_numeric_kmeans_heuristic(x, k, open_ends=False):
	...
	# NA bin
	if any(np.isnan(x)):
		interval_dict['NA'] = Interval(value_type='categorical', value_list=[np.nan])

	# initialization
	percentiles = np.linspace(0., 100., k + 1)

	bounds = np.percentile(np.unique(x), q=percentiles, interpolation='midpoint')
	if len(np.unique(x)) < k:
		warnings.warn('Data does not have enough distinct values for the set number of bins.')
...
def _assign_clusters(x, centers):
	"""Assigns each data value to the corresponding cluster mean.
	   OBSOLETE
	"""
	assignment = {}
	for xx in x:
		...
		try:
			assignment[centers[cidx]].append(xx)
		except KeyError:
			assignment[centers[cidx]] = [xx]
...
def binning_kmeans_dp(x, k):
	"""Dynamic programming solution, computationally too expensive
	   OBSOLETE
	"""
	if isinstance(x, list):
	...
	B = kmeans_dp.fill_dp_matrix(x_sorted, k)
	cluster = kmeans_dp.backtrack(x_sorted, B)
